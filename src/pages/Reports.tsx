...
export default function Reports() {
  ...
  const fetchOutstandingData = async (from = dateFrom, to = dateTo, currency = selectedCurrency) => {
    ...
    if (!error && data && data.length > 0) {
      const formatted = (data as any[]).map(d => {
        ...
        const resolvedNumber = docNumberMap[docId] || d.document_number || d.invoice_number || d.number || d.invoice_no || d.doc_number || d.document_no || '—';
        return {
          document_id: docId,
  ...
  const fetchPaymentsLog = async (from = dateFrom, to = dateTo, currency = selectedCurrency) => {
    ...
    const { data, error } = await query;
    if (!error && data) {
      const docIds = (data as any[]).map(p => p.document_id).filter(Boolean);
      const docNumberMap: Record<string, string> = {};
      ...
      const formatted: PaymentLogEntry[] = (data as any[]).map(p => {
        ...
        const dObj = p.documents;
        let num = docNumberMap[docId] || dObj?.document_number || dObj?.invoice_number || dObj?.number || dObj?.doc_number || '—';
        if (dObj?.prefix && num && num !== '—' && !String(num).startsWith(dObj.prefix)) {
          num = `${dObj.prefix}${num}`;
        }
        return {
          id: p.id,
          ...
          notes: p.notes,
          document_number: num,
          customer_name: dObj?.customers?.name || '—',
