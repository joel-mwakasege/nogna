-- 1. UPDATE REVENUE VIEW TO EXCLUDE TAX (Use subtotal instead of total_amount)
CREATE OR REPLACE VIEW revenue_by_period_view WITH (security_invoker = true) AS
SELECT 
    company_id,
    currency,
    ...
    SUM(COALESCE(subtotal, 0)) AS total_revenue -- Excludes VAT
FROM 
...
-- 2. CREATE NET VAT REPORTING VIEW
...
WITH output_vat AS (
    ...
    GROUP BY company_id, currency, EXTRACT(year FROM issue_date), EXTRACT(month FROM issue_date)
),
input_vat AS (
    SELECT
        e.company_id,
    ...
    GROUP BY e.company_id, c.code, EXTRACT(year FROM e.expense_date), EXTRACT(month FROM e.expense_date)
)
SELECT
...
FROM output_vat o
FULL OUTER JOIN input_vat i
    ON o.company_id = i.company_id AND o.year = i.year AND o.month = i.month AND o.currency = i.currency;
